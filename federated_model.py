...
#  * @Author: zhangqiuhong
#  * @Date: 2025-09-20
#  * @Description: 基于Split Learning理论的联邦Qwen模型架构
#  * 
#  * Split Learning核心思想：
...
class QwenClientEmbedding(nn.Module):
    ...
    def forward(self, input_text: str) -> torch.Tensor:
        ...
        with torch.no_grad():
            embeddings = self.embed_tokens(input_ids)  # [1, seq_len, embedding_dim]
...
class QwenServerModel(nn.Module):
    ...
    def __init__(self,
        ...
        # 获取分隔符token IDs
        self.sep_start_token = self.tokenizer.encode("<|object_ref_start|>", add_special_tokens=False)[0]
        self.sep_end_token = self.tokenizer.encode("<|object_ref_end|>", add_special_tokens=False)[0]
        
        logger.info("✅ 服务端Qwen2.5模型初始化完成:")
        logger.info(f"   模型路径: {model_path}")
        logger.info(f"   嵌入维度: {self.model.model.embed_tokens.embedding_dim}")
        logger.info(f"   词汇表大小: {self.model.model.embed_tokens.num_embeddings}")
        logger.info(f"   分隔符tokens: {self.sep_start_token}, {self.sep_end_token}")
    ...
    def create_federated_input_embeddings(self, 
        ...
        for client_name in sorted(client_embeddings.keys()):
            ...
            client_role_mask = client_role_encoded['attention_mask'].to(self.device)
            ...
            client_emb_batched = client_emb.unsqueeze(0)  # [1, seq_len, embed_dim]
            client_mask = torch.ones((1, client_emb.size(0)), device=self.device)
        ...
        if target_output is not None:
            ...
            assistant_role_ids = assistant_role_encoded['input_ids'].to(self.device)
            assistant_role_emb = self.model.model.embed_tokens(assistant_role_ids)
            assistant_role_mask = assistant_role_encoded['attention_mask'].to(self.device)
        ...
        # 5. 拼接所有embeddings
        combined_embeddings = torch.cat(all_embeddings, dim=1)  # [1, total_seq_len, embed_dim]
        combined_attention_mask = torch.cat(all_masks, dim=1)   # [1, total_seq_len]
        ...
        if is_inference_mode:
            ...
            if seq_len > max_length:
                logger.warning(f"⚠️ 推理序列长度 {seq_len} 超过max_length {max_length}，进行截断")
            ...
            if len(all_token_ids) != seq_len:
                logger.warning(f"⚠️ Token IDs长度 ({len(all_token_ids)}) 与embeddings长度 ({seq_len}) 不匹配")
                if len(all_token_ids) > seq_len:
            ...
            if seq_len > max_length:
                ...
                if output_start_pos is not None and output_start_pos >= max_length:
                    logger.warning(f"⚠️ Output开始位置 {output_start_pos} 超过max_length {max_length}，调整为None")
                    output_start_pos = None
                ...
                pad_embeddings = torch.zeros((1, pad_length, embed_dim), device=self.device)
                ...
                combined_attention_mask = torch.cat([combined_attention_mask, pad_mask], dim=1)
            ...
            assert labels.size(1) == combined_embeddings.size(1), f"Labels长度 {labels.size(1)} 与embeddings长度 {combined_embeddings.size(1)} 不匹配"
    ...
    def forward_with_embeddings(self, 
        ...
        if labels is not None:
            ...
            if output_start_pos is not None:
                ...
                loss_mask[:, adjusted_output_start_pos:] = True  # 只有output部分为True
                
                # 只选择output部分的logits和labels
                masked_logits = shift_logits[loss_mask]  # [num_output_tokens, vocab_size]
        ...
        try:
            with torch.no_grad():
                ...
                for step in range(max_new_tokens):
                    ...
                    position_ids = torch.arange(current_seq_len, device=self.device).unsqueeze(0)
                    ...
                    # 添加新token的embedding
                    next_token_embedding = self.model.model.embed_tokens(next_token_id.unsqueeze(0))
                    current_embeddings = torch.cat([current_embeddings, next_token_embedding], dim=1)
                    current_mask = torch.cat([current_mask, torch.ones((1, 1), device=self.device)], dim=1)
                ...
                if generated_tokens:
                    ...
                    generated_text_clean = self.tokenizer.decode(generated_tokens, skip_special_tokens=True)
                    ...
                    if generated_text_clean.strip():
                        ...
                        filtered_text = generated_text.replace('<|endoftext|>', '').replace('<|im_start|>', '').replace('<|im_end|>', '')
...
class FederatedQwenSystem(nn.Module):
    ...
    def __init__(self,
        ...
        self.client_embeddings = nn.ModuleDict({
            'port': QwenClientEmbedding('port', model_path, device=device),
            'railway': QwenClientEmbedding('railway', model_path, device=device),
            'customs': QwenClientEmbedding('customs', model_path, device=device)
        })
        
        # 初始化服务端模型
        self.server_model = QwenServerModel(model_path=model_path, device=device)
