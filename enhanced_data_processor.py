...
class EnhancedFederatedDataProcessor:
    ...
    def _check_data_quality(self, df: pd.DataFrame):
        ...
        key_columns = ['CNTR', 'PM', 'FZHZM', 'DZHZM', '目的地省份', '目的地城市', 'CARGO_WGT']
        ...
        for col in key_columns:
            if col in df.columns:
                ...
                logger.info(f"  {col}: 非空率={non_null_ratio:.2%}, 唯一值={unique_count}")
            else:
    ...
    def create_enhanced_samples(self, df: pd.DataFrame, sample_size: int = 2000, 
        ...
        logger.info(f"📊 运输方式比例设置: 铁路运输 {railway_ratio:.1%}, 非铁路运输 {1-railway_ratio:.1%}")
        ...
        logger.info(f"📊 样本分配: 铁路运输 {railway_samples} 个, 非铁路运输 {non_railway_samples} 个")
        ...
        # 统计最终的运输方式分布
        railway_final = sum(1 for s in samples if '**运输方式选择**: 铁路运输' in s.get('output', ''))
        non_railway_final = sum(1 for s in samples if '**运输方式选择**: 公路运输' in s.get('output', ''))
    ...
    def _filter_valid_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"📊 过滤结果: {len(df)} → {len(valid_df)} 行 (保留率: {len(valid_df)/len(df):.2%})")
        return valid_df
    ...
    def _create_single_sample(self, row: pd.Series, sample_idx: int, transport_mode: str = 'railway') -> Optional[Dict]:
        ...
        try:
            ...
            # 铁路专家信息
            railway_text = self._create_railway_expert_text(row, transport_mode)
    ...
    def _create_port_expert_text(self, row: pd.Series) -> str:
        ...
        # 港口作业时间信息
        berth_time = row.get('ETA_to_BerthTime', 16)
        yard_stay = row.get('YardStay_Duration', 48)
...
铁路线路：{origin_station}至{dest_station}线路当前运能紧张。货物品名：{cargo_type}。铁路运输时效：约{transit_hours:.1f}小时，但发车班次有限。建议考虑公路运输替代方案：预计公路距离{distance}公里，运输时间{road_hours:.1f}小时，运价约{road_price:.0f}元。"""
    ...
    def _create_customs_expert_text(self, row: pd.Series) -> str:
        ...
        risk_level_cn = {'LOW': '低风险', 'MEDIUM': '中风险', 'HIGH': '高风险'}.get(risk_level, '低风险')
...
def main():
    ...
    try:
        ...
        # 3. 创建增强样本（控制运输方式比例）
        samples = processor.create_enhanced_samples(df, args.sample_size, args.railway_ratio)
