...
def parse_qwen_sample(sample: dict, enabled_clients: List[str] = None):
    """解析Qwen格式的数据 - 支持增强版和旧版格式，支持客户端过滤"""
    ...
    if 'port_expert' in sample and 'railway_expert' in sample and 'customs_expert' in sample:
        # 增强版格式
        server_instruction = sample.get(
            'server_instruction', "请根据以下信息做出最优运输决策")
        all_client_instructions = {
        ...
        # 只保留启用的客户端
        client_instructions = {
            client: all_client_instructions[client]
            for client in enabled_clients
            if client in all_client_instructions
        }
    ...
    for i in range(1, min(4, len(parts))):  # 最多3个客户端
        ...
        if '<|object_ref_end|>' in client_part:
            ...
            client_name = client_names[i-1] if i - \
                1 < len(client_names) else f'client_{i}'
            all_client_instructions[client_name] = client_content

    # 只保留启用的客户端
    client_instructions = {}
    for client_name in enabled_clients:
        if client_name in all_client_instructions:
            client_instructions[client_name] = all_client_instructions[client_name]
        else:
...
def evaluate_test_accuracy(federated_model, test_samples: List[Dict], max_samples: int = 50, enabled_clients: List[str] = None) -> Dict:
    """在训练过程中快速评估测试集准确率"""
    if not test_samples:
    ...
    eval_samples = random.sample(
        test_samples, min(max_samples, len(test_samples)))
    ...
    with torch.no_grad():
        for sample in eval_samples:
            try:
                server_instruction, client_instructions, expected_output = parse_qwen_sample(
                    sample, enabled_clients)
...
def train_federated_qwen(
    ...
    logger.info(f"🏢 启用的客户端: {enabled_clients}")

    try:
        # 1. 加载联邦模型
        logger.info("🔧 初始化联邦Qwen模型...")
        federated_model = FederatedQwenSystem(
        ...
        if (Path(data_dir) / "enhanced_qwen_train.jsonl").exists():
            ...
            train_file = Path(data_dir) / "qwen_processed" / \
                "qwen_federated_train.jsonl"
            test_file = Path(data_dir) / "qwen_processed" / \
                "qwen_federated_test.jsonl"
        else:
        ...
        for epoch in range(epochs):
                    ...
                    if "loss" in outputs:
                        ...
                        if (step + 1) % gradient_accumulation_steps == 0:
                            ...
                            torch.nn.utils.clip_grad_norm_(
                                federated_model.parameters(), max_grad_norm)
                            ...
                            if test_samples and global_step > 0 and global_step % eval_steps == 0:
                                ...
                                eval_results = evaluate_test_accuracy(
                                    federated_model, test_samples, max_samples=50, enabled_clients=enabled_clients)
                                logger.info(
                                    f"📈 测试集准确率: {eval_results['overall_accuracy']:.2%}")
            ...
            if epoch_valid_steps > 0:
                ...
                logger.info(
                    f"✅ Epoch {epoch+1} 完成, 平均损失: {avg_epoch_loss:.4f}")
        ...
        logger.info(
            f"   - 最终平均损失: {total_loss / valid_steps if valid_steps > 0 else 0:.4f}")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str,
                        default="federated_qwen_output", help="输出目录")
    parser.add_argument("--max_samples", type=int, default=None, help="最大样本数")
    parser.add_argument("--enabled_clients", type=str, nargs='+',
                        default=["port", "railway", "customs"],
                        help="启用的客户端列表 (port, railway, customs)")
